"""
The GATConv code is from dgl repo. In case further change of DGL, I just copy the code.
"""
...
class GATConv(nn.Module):
    ...
    def reset_parameters(self):
        ...
        print(self.attn_l.shape, self.attn_r.shape)
...
class GraphAttnLayer(GATConv):
    def forward(self, graph, feat, output_attn=False):
        ...
        with graph.local_scope():
            ...
            e = self.leaky_relu(graph.edata.pop('e'))
            # compute softmax
            raw_attn = edge_softmax(graph, e)
            graph.edata['a'] = self.attn_drop(raw_attn)
            # message passing
            graph.update_all(fn.u_mul_e('ft', 'a', 'm'),
                             fn.sum('m', 'ft'))
            rst = graph.dstdata['ft']
...
class GraphAttnLayerWithLoss(GATConv):
    ...
    def __init__(self,
                 ...
                 reg=None,
                 **kwargs):
        super().__init__(in_feats, out_feats, num_heads, feat_drop, attn_drop, negative_slope,
        ...
        with graph.local_scope():
            ...
            if self.training:
                ...
                with compl_graph.local_scope():
                    if self.reg == 'attn_smooth_2':
                        # compl_graph.ndata['ft'] = feat_src * (self.attn_l - self.attn_r)
                        compl_graph.ndata['ft'] = feat_src
                    if self.reg == 'attn_s_smooth_3':
                        graph.ndata['ft'] = self.bn(graph.ndata['ft'])
                    # graph.ndata['ft'] is `Z`.
                    # loss = self.loss_fn(graph, feat_src, compl_graph, self.gamma / self.beta) * self.beta
                    loss = self.loss_fn(graph, feat_src, compl_graph, self.gamma)
                    # `graph` will not be used any more in this function,
                    # and it is also in `graph.local_scope()`.
                    return rst, loss
            elif output_attn:
                return rst, raw_attn
            elif out_rep:
                return rst, graph.ndata['ft']
            else:
                return rst
...
def _attn_smoothing(graph: dgl.DGLGraph, *args) -> torch.Tensor:
    feat = args[0]
    # num_heads = args[1]
    with graph.local_scope():
        graph.ndata['ft'] = feat
...
def _attn_s_smoothing(graph: dgl.DGLGraph, *args) -> torch.Tensor:
    compl_graph: dgl.DGLGraph = args[1]
    with compl_graph.local_scope():
        compl_graph.ndata['ft'] = graph.ndata['ft']
        compl_graph.update_all(fn.u_mul_e('ft', 'w', 'm'),
                         fn.sum('m', 'tmp'))
        tmp = compl_graph.ndata['tmp']
        trace = torch.einsum('ibj,ibj->b', graph.ndata['ft'], tmp)
        return trace.sum() / graph.num_nodes()


def _attn_s_smoothing_ortho(graph: dgl.DGLGraph, *args) -> torch.Tensor:
    ...
    # ortho_loss = torch.sum((torch.mm(z.T, z) * mask / n_nodes - torch.eye(n_dim, device=z.device)) ** 2)
